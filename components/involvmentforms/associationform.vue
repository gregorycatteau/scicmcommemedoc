  ...
  <div class="mainwrapper">
    ...
    <div class="containered">
      ...
      <UForm :schema="schema" :state="state" class="schema">
        <UFormGroup label="Nom de l'association" name="enterprisename" eager-validation>
          <UInput v-model="state.enterprisename" placeholder="Par exemple : Les amis de la forêt" />
        </UFormGroup>
        ...
        <UFormGroup label="Domaine d'activité" name="activityname" eager-validation>
          <UInput v-model="state.activityname" placeholder="Par exemple : randonnées dans les bois" />
        </UFormGroup>
...
const schema = z.object({
  ...
  adressname: z.string().min(2, 'Doit contenir au moins 2 caractères').regex(/^\d+\s[A-Za-z\s]+|Lieu dit\s[A-Za-z\s]+|BP\s\d+|[A-Za-z\s]+$/, 'Adresse invalide'),
  postalcode: z.string().length(5, 'Le code postal doit contenir exactement 5 chiffres').regex(/^\d{5}$/, 'Code postal invalide'),
